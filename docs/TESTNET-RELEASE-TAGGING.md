...
2. Create and push an **annotated** tag (example `testnet-v0.1.6`):

   ```bash
   git checkout main
   git pull
   git tag -a testnet-v0.1.6 -m "Testnet node: sync zips with main (QA RPC, VM, receipts)"
   git push origin testnet-v0.1.6
   ```
...
5. Refresh Boing website D1 listing SHAs (either from repo root or `website/`):
   ...
   # or from website/: cd website && node scripts/network-listings-release-sql.mjs testnet-v0.1.6 [--apply]
   ```
