...
**VRF leader election (opt-in):** `--leader-election vrf` / **`BOING_LEADER_ELECTION=vrf`** selects leaders via **`leader_from_vrf(dummy_vrf_output(round))`**. The stub uses BLAKE3 with an ECVRF-shaped proof (`verify_vrf_output`); production ECVRF can replace prove/verify without changing leader indexing.
