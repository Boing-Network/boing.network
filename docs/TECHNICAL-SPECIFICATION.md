...
**VRF leader election (opt-in):** `--leader-election vrf` / **`BOING_LEADER_ELECTION=vrf`** selects leaders via **`leader_from_vrf(dummy_vrf_output(round))`** (shared BLAKE3 round seed so all nodes agree). Production **RFC 9381** ECVRF-EDWARDS25519-SHA512-TAI prove/verify is available as **`ecvrf_prove`** / **`verify_ecvrf_output`** / **`leader_from_ecvrf_proofs`** (Ed25519 validator keys); gossiping per-validator proofs into consensus is a follow-up.
