...
**Equivocation slash + appeal (thin MVP):** Detected equivocation burns **`EQUIVOCATION_SLASH_BPS`** (50%) of the offender’s active stake to the fee burn sink and records a **`SlashRegistry`** entry with appeal deadline **`round + EQUIVOCATION_APPEAL_WINDOW_BLOCKS`** (1000). Operators may **`submit_slash_appeal`** / **`resolve_slash_appeal`** on the node; an approved appeal restores stake from the burn sink. Read-only JSON-RPC: **`boing_listSlashRecords`**. Registry is in-memory (not yet persisted). Regression: `cargo test -p boing-node --test equivocation_slash`.
