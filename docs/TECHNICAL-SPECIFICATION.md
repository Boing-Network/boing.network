...
| **BlockHeight** | `0x40` | 2 | Push the **current block height** from execution context as a 32-byte word (value in the **low 8** bytes, big-endian; high 24 bytes zero). Same value for all txs in the block. Defaults to **0** when the host omits context (e.g. some simulations). |
| **Timestamp** | `0x41` | 2 | Push the **current block timestamp** (Unix seconds, execution context) as a 32-byte word (low **8** bytes BE). Same value for all txs in the block. Defaults to **0** when the host omits context. |
...
| `boing/vrf-proofs` | `bincode(VrfProofGossip)` | Per-validator RFC 9381 ECVRF proofs for VRF leader election. Peers verify ECVRF at the P2P edge before emitting `VrfProofReceived`. |

**Equivocation slash + appeal (thin MVP):** Detected equivocation burns **`EQUIVOCATION_SLASH_BPS`** (50%) of the offender’s active stake to the fee burn sink and records a **`SlashRegistry`** entry with appeal deadline **`round + EQUIVOCATION_APPEAL_WINDOW_BLOCKS`** (1000). Operators may **`boing_submitSlashAppeal`** / **`boing_resolveSlashAppeal`** (or in-process `submit_slash_appeal` / `resolve_slash_appeal`); an approved appeal restores stake from the burn sink. Read-only JSON-RPC: **`boing_listSlashRecords`**. With a data directory, the registry persists as **`slash_registry.json`**. Regression: `cargo test -p boing-node --test equivocation_slash`.

**Liveness slash (thin MVP):** Non-leader validators with a non-empty mempool call **`tick_liveness`** each block interval. If the expected round leader fails to propose within **`LIVENESS_LEADER_TIMEOUT_SECS`** (3× block time) for **`LIVENESS_MISS_THRESHOLD`** (3) consecutive observations, burn **`LIVENESS_SLASH_BPS`** (10%) of that leader’s active stake and record **`SlashReason::Liveness`**. Successful commits clear the proposer’s miss streak. Single-validator sets skip this path. Regression: `cargo test -p boing-node --test liveness_slash`.

**VRF leader election (opt-in):** `--leader-election vrf` / **`BOING_LEADER_ELECTION=vrf`** elects leaders via gossiped **`VrfProofGossip`** on **`boing/vrf-proofs`**: each validator with a signing key proves RFC 9381 ECVRF-EDWARDS25519-SHA512-TAI for the round; when a **full** set of verified proofs is present, **`leader_from_ecvrf_proofs`** (lexicographically smallest output) wins. Until then, all nodes fall back to shared BLAKE3 **`dummy_vrf_output`** so they stay aligned. Regression: `cargo test -p boing-node --test multi_validator_consensus vrf_ecvrf_proofs_override_stub_leader`; `cargo test -p boing-consensus --lib test_vrf_leader_prefers_ecvrf`.
...
**Stake-derived set (opt-in):** `--validator-set stake` / **`BOING_VALIDATOR_SET=stake`** refreshes the consensus validator list from `StateStore::top_stakers(n)` every epoch (`BOING_STAKE_VALIDATOR_EPOCH_LEN`, default 100), requiring **`stake ≥ BOING_STAKE_VALIDATOR_MIN_STAKE`** (default `MIN_VALIDATOR_STAKE` = 10000). The local validator identity is always retained. **Unbonding:** `Unbond` → `pending_unbond` + unlock height; `ClaimUnbond` after `UNBONDING_DELAY_BLOCKS`. **Leader election:** `--leader-election vrf` uses ECVRF proofs when complete, else `dummy_vrf_output`. Regression: `cargo test -p boing-node --test stake_validator_epochs`.
