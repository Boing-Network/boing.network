...
| First-time public testnet RPC + faucet order | [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md); [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md) |
...
| **Native DEX** (pair directory, routers, multihop, discovery helpers) | [BOING-NATIVE-DEX-CAPABILITY.md](BOING-NATIVE-DEX-CAPABILITY.md); [BOING-L1-DEX-ENGINEERING.md](BOING-L1-DEX-ENGINEERING.md); [NATIVE-DEX-FACTORY.md](NATIVE-DEX-FACTORY.md); [NATIVE-DEX-LEDGER-ROUTER.md](NATIVE-DEX-LEDGER-ROUTER.md); [NATIVE-DEX-SWAP2-ROUTER.md](NATIVE-DEX-SWAP2-ROUTER.md); [NATIVE-DEX-MULTIHOP-SWAP-ROUTER.md](NATIVE-DEX-MULTIHOP-SWAP-ROUTER.md) |
...
| Public RPC preflight + go-live doc | **`npm run preflight-rpc`** or **`check-testnet-rpc`** (tutorial + repo root delegate); [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md); **`deploy-native-amm-pool.mjs`** calls **`client.chainHeight()`** before submit; tutorial **`.gitignore`** **`pool.hex`** |
...
| Testnet ops umbrella + Playwright CI + observer poll | [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md), [PLAYWRIGHT-E2E-CI-OPS.md](PLAYWRIGHT-E2E-CI-OPS.md), **`observer-chain-tip-poll`** |
...
| **`boing_getNetworkInfo` + public testnet operator env** | RPC method + SDK **`getNetworkInfo()`**; operator template [**`tools/boing-node-public-testnet.env.example`**](../tools/boing-node-public-testnet.env.example) (**`BOING_CHAIN_ID`**, **`BOING_CHAIN_NAME`**); docs: [RUNBOOK.md](RUNBOOK.md), [PUBLIC-RPC-NODE-UPGRADE-CHECKLIST.md](PUBLIC-RPC-NODE-UPGRADE-CHECKLIST.md), [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md) |
| **Native DEX pair directory** | VM **`native_dex_factory_bytecode`** — `register_pair` / `pairs_count` / `get_pair_at` + **`Log3`**; **`cargo run -p boing-execution --example dump_native_dex_factory`**; SDK **`nativeDexFactory.ts`**, **`nativeDexFactoryPool.ts`** (**`findNativeDexFactoryPoolByTokens`**, **`fetchNativeDexFactoryPairsCount`**), **`nativeDexFactoryLogs.ts`**; tutorial **`npm run deploy-native-dex-directory`** — [NATIVE-DEX-FACTORY.md](NATIVE-DEX-FACTORY.md) |
| **Native DEX indexer Worker + D1 directory API** | [workers/native-dex-indexer](../workers/native-dex-indexer/) — **`GET /v1/directory/meta`**, **`GET /v1/directory/pools`**; SDK **`nativeDexDirectoryApi.ts`**; handoff [HANDOFF_NATIVE_DEX_DIRECTORY_R2_AND_CHAIN.md](HANDOFF_NATIVE_DEX_DIRECTORY_R2_AND_CHAIN.md); verify **`npm run verify-native-dex-directory-worker`** |
| **Native DEX multihop SDK ergonomics** | **`fetchCpRoutingFromDirectoryLogs`** + **`pickFirstMultihopCpRoute`**; **`uniqueSortedTokenHex32FromCpRoute`** + **`includeVenueTokenAccounts`**; **`applyNativeDexMultihopSimulationToContractCallTx`** after simulate — [NATIVE-DEX-MULTIHOP-SWAP-ROUTER.md](NATIVE-DEX-MULTIHOP-SWAP-ROUTER.md) § SDK |
| **Native DEX ledger + multihop routers** | Ledger forwarder **v1–v3** ([NATIVE-DEX-LEDGER-ROUTER.md](NATIVE-DEX-LEDGER-ROUTER.md)); **swap2** + **2–6 hop** bundles ([NATIVE-DEX-SWAP2-ROUTER.md](NATIVE-DEX-SWAP2-ROUTER.md), [NATIVE-DEX-MULTIHOP-SWAP-ROUTER.md](NATIVE-DEX-MULTIHOP-SWAP-ROUTER.md)); SDK **`nativeDexLedgerRouter.ts`**, **`nativeDexSwap2Router.ts`** + CREATE2 predictors in **`create2.ts`** |
| **Native DEX capability summary** | [BOING-NATIVE-DEX-CAPABILITY.md](BOING-NATIVE-DEX-CAPABILITY.md) — what is / is not EVM-parity on L1 |
| **Node RPC: native DEX factory** | **`cargo test -p boing-node --test native_dex_factory_rpc_happy_path`** |
| **dApp token wizard helpers** | **`boing-sdk`:** **`buildReferenceFungibleDeployMetaTx`**, **`buildReferenceNftCollectionDeployMetaTx`**, **`isBoingTestnetChainId`**, **`preflightContractDeployMetaQa`**, **`isBoingNativeAccountIdHex`** — [BOING-DAPP-INTEGRATION.md](BOING-DAPP-INTEGRATION.md) |
...
| Follow-up | Notes |
|-----------|--------|
| **Hosted observer** | **Architecture spec:** [OBSERVER-HOSTED-SERVICE.md](OBSERVER-HOSTED-SERVICE.md) (OBS-1). **Interim:** **`npm run observer-chain-tip-poll`** ([TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md) §7). **Stepping stones:** [examples/observer-ingest-reference](../examples/observer-ingest-reference/), [examples/observer-d1-worker](../examples/observer-d1-worker/), **`tools/observer-indexer-schema.sql`**. |
| **Universal contract deploy registry** | **`workers/deploy-registry-indexer`** (D1 + cron + HTTP/SSE): all **`ContractDeploy*`** predicted addresses + tx ids (**not** DEX pool discovery). **`boing-sdk`** **`universalContractDeployIndex.ts`** (**`boing-sdk@0.3.1+`**). Handoff: [HANDOFF_Universal_Contract_Deploy_Indexer.md](HANDOFF_Universal_Contract_Deploy_Indexer.md). |
...
| **Multi-pool / contract discovery** | **In-repo:** pair directory + **`register_pair`** + SDK **`findNativeDexFactoryPoolByTokens`** + **`Log3`** parsers ([NATIVE-DEX-FACTORY.md](NATIVE-DEX-FACTORY.md), [BOING-NATIVE-DEX-CAPABILITY.md](BOING-NATIVE-DEX-CAPABILITY.md)). **Interim REST catalog:** materialized **`pools[]`** in the [native-dex-indexer](../workers/native-dex-indexer/) Worker (**D1** + **`boing-sdk`** **`fetchNativeDexDirectoryPoolsPage`**). **Still optional at product scale:** subgraph / receipt-level history / observer ([NATIVE-AMM-INTEGRATION-CHECKLIST.md](NATIVE-AMM-INTEGRATION-CHECKLIST.md) A3.3); see handoff §3.2. |
...
| **OPS-1** | **Canonical testnet pool `AccountId`** in [RPC-API-SPEC.md](RPC-API-SPEC.md) + [TESTNET.md](TESTNET.md) + **`boing-sdk`** + **boing.network** site config + boing.finance | Ops deploy + freeze address | **Done (2026-05-21):** pool **`0x7247ddc3…`** — [OPS-CANONICAL-TESTNET-NATIVE-AMM-POOL.md](OPS-CANONICAL-TESTNET-NATIVE-AMM-POOL.md) § Published. **In this repo:** `website/src/config/testnet.ts`, **`boing-sdk`** `CANONICAL_BOING_TESTNET_NATIVE_CP_POOL_HEX`, [`scripts/canonical-testnet-published.manifest.json`](../scripts/canonical-testnet-published.manifest.json). **Outside repo:** confirm **boing.finance** `contracts.js` / env matches. |
| **E2E-1** | **Playwright** + loaded Boing Express extension (optional CI) | [NATIVE-AMM-INTEGRATION-CHECKLIST.md](NATIVE-AMM-INTEGRATION-CHECKLIST.md) | **In-repo harness:** [examples/native-boing-playwright](../examples/native-boing-playwright/) (headed Chromium; manual unlock; skips without `BOING_EXPRESS_EXTENSION_PATH`). **Ops / CI limits:** [PLAYWRIGHT-E2E-CI-OPS.md](PLAYWRIGHT-E2E-CI-OPS.md); default workflow validates install only. |
...
**Now:** **NAMM-1**–**NAMM-3** are implemented in-repo; **OPS-1** hex is published in spec + TESTNET (**2026-05-21**, live RPC **`end_user`**). **VM-1** is documented and **`/api/networks`** exposes **`meta`** for the desktop app. Next typical slices are **VM-1** (apply §6 in VibeMiner), **E2E-1** (Playwright), or boing.finance env alignment, unless product rotates the canonical pool again.
...
| **Stake-derived validator epochs** | **Done (opt-in):** `--validator-set stake` / **`BOING_VALIDATOR_SET=stake`** refreshes from `top_stakers` with **`min_stake`** (default **`MIN_VALIDATOR_STAKE=10000`**, env **`BOING_STAKE_VALIDATOR_MIN_STAKE`**). **Unbonding delay:** `Unbond` moves stake to `pending_unbond` for **`UNBONDING_DELAY_BLOCKS`** (100); `ClaimUnbond` credits balance after unlock. Regression: `cargo test -p boing-node --test stake_validator_epochs`; `cargo test -p boing-execution --test bond_unbond`. |
| **VRF leader election** | **Done:** RFC 9381 ECVRF prove/verify + **`boing/vrf-proofs`** gossip (`VrfProofGossip`). Opt-in `--leader-election vrf` uses **`leader_from_ecvrf_proofs`** when all validators have verified proofs for the round; otherwise shared BLAKE3 `dummy_vrf_output` fallback. |
| **On-chain slashing** | **Done:** equivocation + liveness slash, **`SlashRegistry`** appeals, **`slash_registry.json`** persistence, **`boing_listSlashRecords`** / **`boing_submitSlashAppeal`** / **`boing_resolveSlashAppeal`**. |
| **Fee market v0** | **Done:** `ceil(gas_used × GAS_PRICE / GAS_UNITS_PER_BOING)` plus optional `extra_fixed_fee` / `transfer_amount_bps`; default **100% `PROTOCOL_TREASURY`** via `NetworkFeePolicy` (`network_fee_config.json`). Block emission still goes to the proposer. QA counted-voter payouts from treasury in apply. No EIP-1559 / tip auctions yet. |
...
| Follow-up | Notes |
|-----------|--------|
| **u128-wide math / adjustable on-chain fee** | **Done (VM + v6 + v7):** **`Mul`** is **256×256 → 256**; **v3–v7** store **`swap_fee_bps`**; **v6+** fee admin; **v7** on-chain Uniswap-style **fee-on-input** (`NATIVE_CP_POOL_CREATE2_SALT_V7` / `constant_product_pool_bytecode_v7`). Quotes: Rust **`constant_product_amount_out_fee_on_input*`**, TS **`constantProductAmountOutFeeOnInput*`**. |
...
| **Operator go-live + RPC smoke** | **Done** — [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md), **`preflight-rpc`** / **`check-testnet-rpc.mjs`**, deploy preflight |
...
| **TESTNET-RPC-INFRA + Playwright CI ops doc + observer poll** | **Done** — [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md), [PLAYWRIGHT-E2E-CI-OPS.md](PLAYWRIGHT-E2E-CI-OPS.md), [observer-chain-tip-poll.mjs](../examples/native-boing-tutorial/scripts/observer-chain-tip-poll.mjs) |
...
| **Docs index: native DEX + AMM integration map** | **Done** — [README.md](README.md) § Design & Build lists capability doc, router specs, [NATIVE-AMM-CALLDATA.md](NATIVE-AMM-CALLDATA.md), integration checklist, LP vault/share, E2E smoke, **OPS-1**; [NEXT-STEPS-FUTURE-WORK.md](NEXT-STEPS-FUTURE-WORK.md) § Enhancements + indexer **Multi-pool / contract discovery** |
| **Root `package.json` delegates for tutorial scripts** | **Done** — every **`examples/native-boing-tutorial`** **`npm run`** name is wired at repo root (same script name); [PRE-VIBEMINER-NODE-COMMANDS.md](PRE-VIBEMINER-NODE-COMMANDS.md) §4, [README.md](../README.md) Quick Start, tutorial [README.md](../examples/native-boing-tutorial/README.md) |
| **dApp deploy wizard DX (`boing-sdk`)** | **Done** — **`buildReferenceFungibleDeployMetaTx`**, **`buildReferenceNftCollectionDeployMetaTx`**, **`isBoingTestnetChainId`** / **`normalizeBoingChainIdHex`**, **`preflightContractDeployMetaQa`**, **`isBoingNativeAccountIdHex`**; [BOING-DAPP-INTEGRATION.md](BOING-DAPP-INTEGRATION.md) § **One wizard, two backends**; [BOING-CANONICAL-DEPLOY-ARTIFACTS.md](BOING-CANONICAL-DEPLOY-ARTIFACTS.md) § **Wizard shortcut** |
...
**Still open (larger):** hosted observer **scale-out read plane** (horizontal read replicas, multi-tenant product hardening — architecture in [OBSERVER-HOSTED-SERVICE.md](OBSERVER-HOSTED-SERVICE.md)); **in-repo D1 worker** already ships **reorg rewind**, read routes, optional **`BOING_READ_CACHE_MAX_AGE`**, and Vitest coverage under **`examples/observer-d1-worker/`**. Unattended Playwright + extension on **shared** GitHub runners. **OPS-1:** canonical pool **`0x7247ddc3…`** is in [RPC-API-SPEC.md](RPC-API-SPEC.md) / [TESTNET.md](TESTNET.md) (**2026-05-21**); align **boing.finance** production env. **Done:** CI workflow **`.github/workflows/boing-sdk-rpc-integration.yml`** builds **`boing-node`**, starts it on **8545**, runs **`npm run probe-rpc`**, **`npm run verify`** (**`BOING_EXPECT_FULL_RPC=1`**: discovery + **`probeBoingRpcCapabilities`** 6/6 + **`planIndexerCatchUp`** live test), then **`examples/native-boing-tutorial`** **`npm run preflight-rpc`**, **`indexer-ingest-tick`**, **`native-amm-print-contract-call-tx`** parse check. **Done:** **`.github/workflows/observer-ingest-d1.yml`** — **`examples/observer-d1-worker`** **`wrangler deploy --dry-run`** + **`tsc`**, **`examples/observer-ingest-reference`** ingest scripts **`node --check`**. **Done:** **`.github/workflows/canonical-pool-public-rpc.yml`** — daily **`check-canonical-pool`** + **`verify-public-testnet-rpc`** + **`verify-qa-alignment`**. Triggers include **`examples/native-boing-tutorial/**`**; repo root **`npm run indexer-ingest-tick`** delegates to the tutorial package (run **`npm install`** there first if needed).
