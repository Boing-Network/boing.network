...
| **On-chain slashing** | **Done (thin MVP + gossip + appeal + liveness):** equivocation slash + gossip + **`SlashRegistry`** appeals; **liveness** via leader-timeout **`tick_liveness`** (`LIVENESS_MISS_THRESHOLD` / `LIVENESS_SLASH_BPS`). **Still open:** persisted registry / public RPC for appeals; production timeout tuning. |
