...
| **On-chain slashing** | **Done (thin MVP + gossip):** local + gossiped `EquivocationEvidence` on `boing/equivocation`; peers verify both votes and slash **`EQUIVOCATION_SLASH_BPS`** (50%) once per `(validator, round)`. **Still open:** appeal flow wiring, liveness slashing. |
