...
| **VRF leader election** | **Done:** RFC 9381 **ECVRF-EDWARDS25519-SHA512-TAI** via `ecvrf_prove` / `verify_ecvrf_output` / `leader_from_ecvrf_proofs` (Ed25519 seeds = validator keys). Opt-in `--leader-election vrf` still uses shared BLAKE3 `dummy_vrf_output` for deterministic multi-node agreement until per-validator proofs are gossiped. |
