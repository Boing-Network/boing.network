...
| **VRF leader election** | **Done (opt-in stub):** `--leader-election vrf` / **`BOING_LEADER_ELECTION=vrf`** uses `dummy_vrf_output(round)` + `leader_from_vrf` (deterministic; not production ECVRF). Default remains round-robin. |
| **On-chain slashing** | **Done (thin MVP + gossip + appeal):** local + gossiped `EquivocationEvidence`; slash **`EQUIVOCATION_SLASH_BPS`** (50%) once per `(validator, round)`; **`SlashRegistry`** + `submit_slash_appeal` / `resolve_slash_appeal` (approved → restore from burn sink; window **`EQUIVOCATION_APPEAL_WINDOW_BLOCKS`**). **Still open:** liveness slashing, persisted registry / public RPC for appeals. |
