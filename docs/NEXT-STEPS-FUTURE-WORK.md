...
| **VRF leader election** | **Done (opt-in stub + verify path):** `--leader-election vrf` / **`BOING_LEADER_ELECTION=vrf`** uses BLAKE3 VRF-shaped `dummy_vrf_output` / `verify_vrf_output` + `leader_from_vrf`. **Still open:** production ECVRF over Ed25519 (API shaped for drop-in). |
...
| Follow-up | Notes |
|-----------|--------|
| **u128-wide math / adjustable on-chain fee** | **Done (VM + v6 + fee-on-input helper):** **`Mul`** is **256×256 → 256**; **v3/v4/v6** store **`swap_fee_bps`**; **v6** fee admin. Off-chain Uniswap-style **fee-on-input** quotes: Rust **`constant_product_amount_out_fee_on_input*`**, TS **`constantProductAmountOutFeeOnInput*`**. **Still open:** optional bytecode revision that applies fee-on-input on-chain. |
