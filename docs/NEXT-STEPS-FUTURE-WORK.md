...
| **On-chain slashing** | **Done (thin MVP + gossip + appeal + liveness + read RPC):** equivocation/liveness slash + **`SlashRegistry`** appeals; **`boing_listSlashRecords`**. **Still open:** persisted registry; public write RPC for appeal submit/resolve. |
