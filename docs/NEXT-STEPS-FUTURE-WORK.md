...
| **On-chain slashing** | **Done (thin MVP):** local equivocation (`ConsensusError::Equivocation`) burns **`EQUIVOCATION_SLASH_BPS`** (50%) of active stake to **`FEE_BURN_SINK`** via `slash_equivocation_stake` in `on_consensus_vote`. **Still open:** gossip evidence, appeal flow wiring, liveness slashing. |
