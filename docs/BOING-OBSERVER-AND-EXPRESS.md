...
- **The explorer app itself** — a separate frontend (e.g. Next.js, Remix, Astro, or Vite) that:
  ...
  - Reads from the Boing JSON-RPC (e.g. testnet RPC URL from env): `boing_chainHeight`, `boing_getBlockByHeight`, `boing_getBlockByHash`, `boing_getBalance`, `boing_getAccount`, optional `boing_getTransactionReceipt` / bounded `boing_getLogs` for tx/event views, and for QA transparency: `boing_qaPoolList`, `boing_qaPoolConfig`, `boing_getQaRegistry` (see `/qa` on boing.observer). Reviewer Allow/Reject on Unsure deploys is an explorer-gated call to `boing_qaPoolVote` (operator header stays on the server).
  - Implements: network selector (Testnet/Mainnet), home (chain height + latest blocks), block detail (by height/hash), account page (balance, nonce, stake), search (height / hash / address), **PDF embeds for written docs** (six pillars on `/about`), and “Protocol QA Passed” for ContractDeploy + QA explainer in footer/About.
...
**Before the explorer is deployed:** operators can still **monitor** the chain tip over JSON-RPC — see [TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md) §3 (`npm run observer-chain-tip-poll`, probes, indexer scripts). That is **not** a substitute for **boing.observer** UX or durable indexing.
