...
- **Pool id:** Prefer **`fetchNativeDexIntegrationDefaults(client)`** from **`boing-sdk`** — it uses **`end_user.canonical_native_cp_pool`** from the node when set (**`BOING_CANONICAL_NATIVE_CP_POOL`**), else your overrides. Historical embedded **6913** constants apply only when network info is omitted (offline). A live RPC with null canonical fields means the hosted chain has not published a pool. See [RPC-API-SPEC.md](RPC-API-SPEC.md) § Native constant-product AMM and [OPS-CANONICAL-TESTNET-NATIVE-AMM-POOL.md](OPS-CANONICAL-TESTNET-NATIVE-AMM-POOL.md) for rotations.
...
- **Browser smoke:** [NATIVE-AMM-INTEGRATION-CHECKLIST.md](NATIVE-AMM-INTEGRATION-CHECKLIST.md) (Boing Express + boing.finance).

### Native DEX: defaults, directory logs, injected wallet

- **One discovery facade:** **`createNativeDexDiscovery(client, { overrides?, directoryBaseUrl?, useProcessEnvOverrides? })`** picks the best pool source (**L1 `boing_listDexPools` → HTTP Worker directory → none**), exposes **`collectAllPools`**, directory snapshot / resolve helpers, and merged defaults. Prefer this over calling the layered helpers directly. Env directory base: **`BOING_NATIVE_DEX_DIRECTORY_BASE_URL`** (also `VITE_` / `REACT_APP_` variants) via **`readNativeDexDirectoryBaseUrlFromProcessEnv`**.
- **TS starter:** [examples/boing-dapp-starter](../examples/boing-dapp-starter/) — `npm start` runs preflight + discovery against **`BOING_RPC_URL`** (default public testnet).
