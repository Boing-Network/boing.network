...
| **Publishing the canonical pool id** | **Hosted Fly testnet:** **unset** (`end_user.canonical_native_cp_pool` is null). Read live RPC; do not hardcode the historical **`0x7247ddc3…`** tunnel-ledger id. VibeMiner does **not** store the pool id. **Future** rotations: [OPS-CANONICAL-TESTNET-NATIVE-AMM-POOL.md](OPS-CANONICAL-TESTNET-NATIVE-AMM-POOL.md) ([TESTNET-RPC-INFRA.md](TESTNET-RPC-INFRA.md) §2–3). |
