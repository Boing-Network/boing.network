 ...
 * Body: { account_id_hex, role, email?, discord_handle?, github_username?, node_multiaddr? }
 */
export async function onRequestPost(context) {
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO portal_registrations (account_id_hex, role, email, discord_handle, github_username, node_multiaddr, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      ...
      .bind(account_id_hex, role, email, discord_handle, github_username, node_multiaddr, now, now)
      .run();
