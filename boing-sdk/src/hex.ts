...
export function hexToBytes(hex: string): Uint8Array {
  ...
  if (raw.length % 2 !== 0) throw new Error('Invalid hex: length must be even');
  if (!HEX_RE.test(raw)) throw new Error('Invalid hex: expected 0-9, a-f, A-F');
 ...
 */
export function validateHex32(hex: string): string {
  const normalized = ensureHex(hex);
  const raw = normalized.slice(2);
  if (raw.length !== 64) throw new Error(`Expected 32 bytes (64 hex chars), got ${raw.length} hex chars`);
  if (!HEX_RE.test(raw)) throw new Error('Invalid hex: expected 0-9, a-f, A-F');
  return normalized;
}
...
export function hexToAccountId(hex: string): Uint8Array {
  const bytes = hexToBytes(hex);
  if (bytes.length !== 32) throw new Error('AccountId must be 32 bytes hex');
  return bytes;
}
