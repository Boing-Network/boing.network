...
function ensure0x32(hex: string): string {
  const t = hex.trim();
 ...
 */
export function displayChainTitle(info: NetworkInfo): string {
...
export function explorerBaseUrl(info: NetworkInfo): string | undefined {
...
export function faucetUrl(info: NetworkInfo): string | undefined {
 ...
 */
export function explorerTxUrl(info: NetworkInfo, txHashHex: string): string | undefined {
 ...
 * Returns **`undefined`** if no explorer base is configured.
 */
export function explorerAccountUrl(info: NetworkInfo, accountIdHex: string): string | undefined {
  const base = explorerBaseUrl(info);
  if (base == null) return undefined;
 ...
 */
export function formatSupportHint(userMessage: string, requestId?: string | null): string {
