 ...
 */
export const NATIVE_DEX_FACTORY_COUNT_KEY_HEX =
  `0x${'00'.repeat(16)}424f494e4744455800000000ffffffff` as const;

/** `read` / `write`: signer + directory contract (parallel scheduling minimum). */
export function buildNativeDexFactoryAccessList(
  senderHex32: string,
  factoryHex32: string
...
export function buildNativeDexFactoryContractCallTx(
  senderHex32: string,
  factoryHex32: string,
  calldataHex: string
): {
...
export function mergeNativeDexFactoryAccessListWithSimulation(
  senderHex32: string,
  factoryHex32: string,
  sim: SimulateResult
): { read: string[]; write: string[] } {
...
export function decodeNativeDexFactoryPairsCountReturnData(returnDataHex: string): bigint {
 ...
 */
export function decodeNativeDexFactoryGetPairAtReturnData(returnDataHex: string): {
  tokenAHex: string;
  tokenBHex: string;
  poolHex: string;
} {
  ...
  const word = (i: number) => validateHex32(`0x${raw.slice(i * 64, i * 64 + 64)}`);
  return {
    tokenAHex: word(0),
    tokenBHex: word(1),
    poolHex: word(2),
...
export async function fetchNativeDexFactoryPairsCount(
  client: BoingClient,
  factoryHex32: string
): Promise<bigint> {
...
function sameAccountHex(a: string, b: string): boolean {
 ...
 */
export async function findNativeDexFactoryPoolByTokens(
  client: BoingClient,
  factoryHex32: string,
  tokenAHex32: string,
  tokenBHex32: string,
  options: FindNativeDexFactoryPoolOptions
): Promise<string | null> {
  ...
  for (let i = 0n; i < n; i++) {
    ...
    try {
      ...
      const hitDirect =
        sameAccountHex(tokenAHex32, row.tokenAHex) && sameAccountHex(tokenBHex32, row.tokenBHex);
      const hitSwap =
        sameAccountHex(tokenAHex32, row.tokenBHex) && sameAccountHex(tokenBHex32, row.tokenAHex);
