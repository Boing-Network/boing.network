...
export type BoingNewHeadsWsState =
  | 'idle'
  | 'connecting'
  | 'subscribed'
  | 'reconnecting'
...
export function boingRpcWebSocketUrl(rpcBaseUrl: string, path = '/ws'): string {
...
export class BoingNewHeadsWs {
  ...
  private state: BoingNewHeadsWsState = 'idle';
  ...
  constructor(
    ...
    private readonly options: BoingNewHeadsWsOptions = {},
  ) {}

  private setState(s: BoingNewHeadsWsState): void {
    this.state = s;
  ...
  connect(): void {
    const WS = globalThis.WebSocket as typeof WebSocket | undefined;
    if (WS == null) {
    ...
    ws.onmessage = (ev: MessageEvent) => {
      ...
      let j: unknown;
      try {
        j = JSON.parse(ev.data) as unknown;
      } catch {
        return;
      }
