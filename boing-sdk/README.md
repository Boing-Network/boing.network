...
- **BoingClient** — typed methods for all RPCs (32-byte account/hash params are validated locally before sending):
  ...
  - **Universal contract deploy index** — **`rpcTransactionJsonToTransactionInput`**, **`transactionIdFromUnsignedRpcTransaction`**, **`extractUniversalContractDeploymentsFromBlock`** / **`extractUniversalContractDeploymentsFromBlockJson`**, **`CONTRACT_DEPLOY_INIT_CODE_MARKER`**, row types (`universalContractDeployIndex.ts`; shipped **0.3.1+**). Optional hosted materialization + REST/SSE: [HANDOFF_Universal_Contract_Deploy_Indexer.md](../docs/HANDOFF_Universal_Contract_Deploy_Indexer.md), **`workers/deploy-registry-indexer`**.
